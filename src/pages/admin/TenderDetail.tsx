...
function EmailModal({ tender, tenderItems, onClose }: EmailModalProps) {
  ...
  const handleSendEmails = async () => {
    try {
      ...
      // Call the function to send emails to all wholesalers
          ...
          <div className="space-y-4">
            ...
            <div className="bg-amber-50 p-4 rounded-md">
              <p className="text-sm text-amber-700">
                <strong>Note :</strong> L'email ne contiendra aucune information personnelle concernant le pharmacien. Les grossistes verront uniquement les produits demandés, les quantités et la wilaya de livraison.
              </p>
            </div>
...
export function TenderDetail() {
  ...
  const [messages, setMessages] = useState<(TenderMessage & { user: UserType })[]>([]);
  ...
  const [editingPharmacist, setEditingPharmacist] = useState(false);
  const [selectedPharmacist, setSelectedPharmacist] = useState<{ id: string; company_name: string } | null>(null);
  ...
  async function fetchTenderDetails() {
    try {
      ...
      // Fetch tender
      const { data: tenderData, error: tenderError } = await supabaseAdmin
        .from('tenders')
        .select('*')
        .eq('id', id)
        .single();
      ...
      // If there's a pharmacist_id, fetch the pharmacist details
      if (tenderData.pharmacist_id) {
        const { data: pharmacistData } = await supabaseAdmin
          .from('users')
          .select('id, company_name')
          .eq('id', tenderData.pharmacist_id)
          .single();
          
        if (pharmacistData) {
          setSelectedPharmacist(pharmacistData);
        }
      }
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabaseAdmin
        ...
        .insert({
          tender_id: id,
          user_id: tender.pharmacist_id, // Send as the pharmacist
  ...
  const handleCloneTender = async () => {
    ...
    try {
      ...
      const { data: newTender, error: tenderError } = await supabaseAdmin
        .from('tenders')
        .insert({
          pharmacist_id: tender.pharmacist_id,
          title: `${tender.title} (copie)`,
          deadline: newDeadline.toISOString(),
          wilaya: tender.wilaya,
          is_public: tender.is_public,
          status: 'open',
          admin_facebook_link: tender.admin_facebook_link,
          admin_facebook_profile: tender.admin_facebook_profile
        })
        .select()
        .single();
      ...
      navigate(`/admin/tenders/${newTender.id}`);
    } catch (error) {
  ...
  const handleCreateOrder = async (responseId: string) => {
    if (!tender || !selectedPharmacist) return;
    ...
    try {
      ...
      // Get pharmacist details for the email
      const { data: pharmacistData, error: pharmacistError } = await supabaseAdmin
        .from('users')
        .select('*')
      ...
      try {
        await sendOrderNotification(
          ...
          response.wholesaler.email,
          {
            ...
            pharmacist_name: pharmacistData.company_name,
            pharmacist_email: pharmacistData.email,
            pharmacist_phone: pharmacistData.phone,
            pharmacist_address: pharmacistData.address,
            pharmacist_wilaya: pharmacistData.wilaya,
            order_id: orderData.id,
            tender_id: tender.id,
  ...
  const handleUpdatePharmacist = async () => {
    ...
    try {
      ...
      const { error } = await supabaseAdmin
        .from('tenders')
        .update({ pharmacist_id: selectedPharmacist.id })
        .eq('id', tender.id);
      
      if (error) throw error;
      
      setTender({ ...tender, pharmacist_id: selectedPharmacist.id });
      setEditingPharmacist(false);
    } catch (error) {
      console.error('Error updating pharmacist:', error);
      setError('Erreur lors de la mise à jour du pharmacien');
    } finally {
      setSavingPharmacist(false);
    }
  };

  const loadPharmacists = async (inputValue: string) => {
    try {
      let query = supabaseAdmin
        .from('users')
      ...
      if (inputValue) {
        query = query.or(`company_name.ilike.%${inputValue}%,email.ilike.%${inputValue}%`)
      }
  
      const { data, error } = await query.limit(10)
      if (error) throw error
  
      // on retourne les options au format react-select
      return (data || []).map(u => ({
        value:   u.id,
        label:   `${u.company_name} (${u.email})`,
        raw:     u
      }))
    } catch (err) {
      console.error('loadPharmacists error:', err)
      return []
    }
  }
            ...
            <dl className="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
              ...
              {tender.admin_facebook_link && (
                <div className="sm:col-span-2">
                  <dt className="text-sm font-medium text-gray-500">Lien Facebook</dt>
                  <dd className="mt-1 text-sm text-gray-900">
                    <a 
                      href={tender.admin_facebook_link} 
                      target="_blank" 
              ...
              {tender.admin_facebook_profile && (
                <div className="sm:col-span-2">
                  <dt className="text-sm font-medium text-gray-500">Profil Facebook</dt>
                  <dd className="mt-1 text-sm text-gray-900">{tender.admin_facebook_profile}</dd>
                </div>
              )}
              <div className="sm:col-span-2">
              ...
              <div className="sm:col-span-2">
                <dt className="text-sm font-medium text-gray-500">Pharmacien</dt>
                <dd className="mt-1 text-sm text-gray-900">
                  {editingPharmacist ? (
                    <div className="space-y-2">
                      <AsyncSelect
                        ...
                        value={selectedPharmacist ? {
                          ...
                          label: selectedPharmacist.company_name
                        } : null}
                        placeholder="Rechercher un pharmacien…"
                        noOptionsMessage={() => "Aucun pharmacien trouvé"}
                      />
                      ...
                      <div className="flex justify-end space-x-2">
                        <button
                          type="button"
                          onClick={() => setEditingPharmacist(false)}
                        ...
                        <button
                          ...
                          disabled={!selectedPharmacist || savingPharmacist}
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        {selectedPharmacist ? (
                          <span>{selectedPharmacist.company_name}</span>
                        ) : (
                          <span className="text-gray-500">Non assigné</span>
                        )}
                      </div>
                      <button
                        onClick={() => setEditingPharmacist(true)}
              ...
              <div key={response.id} className="p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex space-x-2">
                    ...
                    {tender.status === 'open' && selectedPharmacist && (
                      <button
          ...
          <div className="space-y-4 max-h-96 overflow-y-auto mb-4">
            {messages.length === 0 ? (
              ...
              messages.map((message) => {
                const isPharmacist = message.user_id === tender.pharmacist_id;
                    ...
                    >
                      <div className="flex items-center mb-1">
                        <span className="text-xs font-medium">
                          {isPharmacist ? 'Pharmacien' : message.user.company_name}
                        </span>
          ...
          <form onSubmit={handleSendMessage} className="mt-4">
            <div className="flex space-x-3">
              <input
                ...
                placeholder="Écrivez un message en tant que pharmacien..."
  ...
  function handleExportTender() {
    ...
    try {
      ...
      const tenderInfoData = [
        ['Titre', tender.title],
        ['Pharmacien', tender.pharmacist_id ? 'Pharmacien' : 'Admin'],
        ['Wilaya', tender.wilaya],
