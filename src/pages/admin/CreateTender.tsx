  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data: tender, error: tenderError } = await supabaseAdmin
        .from('tenders')
        .insert({
          pharmacist_id: user?.id, // Will be updated with a real pharmacist ID
          title,
          deadline,
          wilaya,
          is_public: isPublic,
          status: 'open',
          admin_facebook_link: facebookLink || null,
          admin_facebook_profile: facebookProfile || null
        })
        .select('id')
        .single();
      ...
      // Redirect to tender management page
      ...
      <form onSubmit={handleSubmit} className="bg-white shadow rounded-lg p-6">
        <div className="space-y-6">
          ...
          <div className="bg-blue-50 p-4 rounded-lg">
            <h3 className="text-md font-medium text-blue-800 mb-2">Informations Facebook (visibles uniquement par l'admin)</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label htmlFor="facebookLink" className="block text-sm font-medium text-gray-700">
                  Lien de la demande Facebook
                </label>
                <input
                  type="url"
                  id="facebookLink"
              ...
              <div>
                <label htmlFor="facebookProfile" className="block text-sm font-medium text-gray-700">
                  Profil Facebook du demandeur
                </label>
                <input
                  type="text"
                  id="facebookProfile"
